...
- 支持 `Ctrl+Enter` 快捷发送；
- 关闭主窗口后继续在系统托盘运行；
- 托盘状态下按 `Ctrl+Alt+Z` 唤起无边框、只有文本框的快速发送浮层；
...
- 点击主窗口右上角的关闭按钮只会隐藏窗口，程序会继续在系统托盘运行；
- 在任意程序中按 `Ctrl+Alt+Z`，可唤起极简快速发送窗；
- 快速发送浮层没有标题栏和其他按钮：按 `Enter` 发送，`Shift+Enter` 换行；
- 提交后浮层会立即隐藏；按 `Esc` 可取消并隐藏；
- 左键点击托盘图标可恢复主窗口；右键托盘图标可打开主窗口、快速发送或彻底退出。

全局快捷键只在程序运行期间注册，不会永久修改 Windows 设置。如果快捷键已被其他
程序占用，仍可从托盘右键菜单打开快速发送窗。
...
确认挂件仍在系统托盘运行。如果启动时提示快捷键已被占用，请关闭占用该组合键的程序，
重新启动挂件；也可以直接从托盘右键菜单选择“快速发送”。
